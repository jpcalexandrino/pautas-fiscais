...
import { mapFromDb } from '@features/faturas/mappers';
import { useClient } from '@features/clients/context/ClientContext';
import { useEquipmentContext } from '@features/equipment/context/EquipmentContext';
...
export default function ReportsPage() {
  ...
  const { state, saveRows, getFaturas } = useFatura();
  const { getClients } = useClient();
  const { getEquipmentByClient } = useEquipmentContext();
  ...
  if (selectedData.length === 0) {
    return <PDFEmptyState onBack={() => navigate({ to: '/data' as any })} />;
  }
