...
import { CSV_FIELDS, DEFAULT_VISIBLE_COLUMNS } from '../utils/constants';
...
import { ColumnCustomizer } from '@/components/data/ColumnCustomizer';
import { DataPagination } from '@/components/data/DataPagination';

export default function DataPage() {
  const navigate = useNavigate();
  const { state, clearData } = useData();
  ...
  if (rows.length === 0) {
    return <DataEmptyState onNavigateToImport={() => navigate({ to: '/importacao' })} />;
  }
    ...
    <div className="space-y-8 pb-10 animate-fade-in">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button
            variant="outline"
            onClick={() => { clearData(); navigate({ to: '/importacao' }); }}
